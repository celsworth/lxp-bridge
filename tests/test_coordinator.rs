...
async fn publishes_read_hold_mqtt() {
    ...
    let tf = async {
        ...
        let packet = Packet::TranslatedData(lxp::packet::TranslatedData {
            datalog: config.borrow().inverters[0].datalog,
            device_function: lxp::packet::DeviceFunction::ReadHold,
            inverter: config.borrow().inverters[0].serial,
            register: 12,
            values: vec![22, 6],
        });
        channels
        ...
        assert_eq!(
            ...
            mqtt::ChannelData::Message(mqtt::Message {
                topic: format!("{}/hold/12", config.borrow().inverters[0].datalog),
